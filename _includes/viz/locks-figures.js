 ...
 */
(function () {
  // The static gap explainer and the transaction simulation share a table model
  // but have independent builders and interactions.
  const gapNodes = [...document.querySelectorAll(".lx-gap")];
  const simulationNodes = [...document.querySelectorAll(".lx-sim")];
  if (!gapNodes.length && !simulationNodes.length) return;
  ...
  // ====================================================================
  //  Gap explainer — hover or focus the static table
  // ====================================================================
  ...
  // ====================================================================
  //  Transaction simulation — two isolation levels racing
  //
  ...
  // ---- Page lifecycle --------------------------------------------------
  // These figures have no external data to wait on. Builders finish their full
  // synchronous scaffold before the reserve can be released.
  function mount(nodes, buildFigure) {
    nodes.forEach((node) => {
      buildFigure(node);
  ...
  mount(gapNodes, buildGap);
  mount(simulationNodes, buildSim);
})();
