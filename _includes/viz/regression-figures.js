 ...
 */
(function () {
  const varianceNodes = [...document.querySelectorAll(".rg-variance[data-src]")];
  if (!varianceNodes.length) return;
  ...
  // Loading, visibility, theme lookup, and canvas setup stay separate from
  // the variance figure's scaffold, drawing, and slider behavior below.
  const cache = {};
  function load(src) {
  ...
  // ---- Page lifecycle --------------------------------------------------
  function mountWhenVisible(nodes) {
    nodes.forEach((node) => {
      const src = node.dataset.src;
      whenVisible(node, () => load(src).then((data) => buildVariance(node, data))
        .catch((error) => {
  ...
  mountWhenVisible(varianceNodes);
})();
