 ...
 */
(function () {
  // This file has two independent figures. Keep their mount lists separate so
  // the page-level lifecycle below reads as plainly as their builders do.
  const waveNodes = [...document.querySelectorAll(".tz-wave[data-src]")];
  const clockNodes = [...document.querySelectorAll(".tz-clock[data-src]")];
  if (!waveNodes.length && !clockNodes.length) return;
  ...
  // Data loading, theme lookup, visibility, and canvas setup live together.
  // The builders below own only their DOM, geometry, drawing, and events.
  const cache = {};
  function load(src) {
  ...
  // ====================================================================
  //  Wave figure — every city firing one local-time promise
  // ====================================================================
  function buildWave(node, d) {
  ...
  // ====================================================================
  //  Clock figure — "The Clock That Skips and Stutters" (gap + fold)
  ...
  // ---- Page lifecycle --------------------------------------------------
  // The loader stays outside each builder: fetch only near the viewport, share
  // one request per data URL, and give the reader a useful failure message.
  function mountWhenVisible(nodes, buildFigure, fallback) {
    nodes.forEach((node) => {
      const src = node.dataset.src || "timezones.json";
      whenVisible(node, () => load(src).then((data) => buildFigure(node, data))
        .catch((error) => {
  ...
  mountWhenVisible(waveNodes, buildWave, "Couldn’t load the time-zone data.");
  mountWhenVisible(clockNodes, buildClock, "Couldn’t load the clock data.");
})();
