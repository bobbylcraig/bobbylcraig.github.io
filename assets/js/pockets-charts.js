...
(function () {
  ...
  function editorialCopy(node, key) { return node.dataset[`viz${key}`] || ""; }
  ...
  function buildTimeseries(node, d) {
    ...
    const chart = makeChart(node, {
      title: editorialCopy(node, "Title"),
      aspect: 0.52, maxH: 320,
      legend: () => [
        { label: "All posts", color: colAll() },
        { label: "Lost-item posts", color: colLost() },
      ],
      caption: editorialCopy(node, "Caption"),
      paint: (ctx, W, H, reveal) => {
        const soft = cssVar("--text-soft", "#6a6a6a");
        const grid = cssVar("--border", "#d0d0d0");
        const padL = 38, padR = 30, padT = 12, padB = 34;
        const plotW = W - padL - padR, plotH = H - padT - padB;
        const ymaxAll = niceCeil(Math.max(...data.map((p) => p.all)));
        const ymaxLost = niceCeil(Math.max(...data.map((p) => p.lost)));
        const X = (i) => padL + (data.length === 1 ? 0 : (i / (data.length - 1)) * plotW);
        const Yall = (v) => padT + (1 - v / ymaxAll) * plotH;
        const Ylost = (v) => padT + (1 - v / ymaxLost) * plotH;
        ...
        for (let g = 0; g <= gn; g++) {
          ...
          ctx.fillText(String(Math.round((ymaxAll / gn) * g)), padL - 6, y);
          ctx.fillStyle = colLost(); ctx.textAlign = "left";
          ctx.fillText(String(Math.round((ymaxLost / gn) * g)), W - padR + 6, y);
        }
        ...
        [
          ["all", colAll(), Yall],
          ["lost", colLost(), Ylost],
        ].forEach(([key, color, scale]) => {
          ctx.strokeStyle = color; ctx.lineWidth = 2.2; ctx.lineJoin = "round";
          ctx.beginPath();
          data.forEach((p, i) => { const x = X(i), y = scale(p[key]); i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); });
          ctx.stroke();
        });
        ctx.restore();
      },
  ...
  function buildGender(node, d) {
    ...
    title.textContent = editorialCopy(node, "Title");
    ...
    const cap = el("p", "vz-caption");
    cap.textContent = editorialCopy(node, "Caption");
    node.appendChild(cap);
  ...
  // scaffold; the mount keeps its reserve as a cross-platform geometry floor.
  function mountCharts(nodes) {
    nodes.forEach((node) => {
      ...
      load(src).then((data) => {
        // viz-mount leaves an editorial no-JavaScript caption in the initial
        // HTML. The hydrated chart owns its matching caption, so replace that
        // fallback rather than appending a duplicate beneath the figure.
