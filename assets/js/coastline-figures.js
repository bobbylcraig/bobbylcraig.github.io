/* Coastline figures — "The 10-Terabyte Coastline" post.
 *
 ...
 */
(function () {
  // The article has two independent figures that share one data file. Keep the
  // mount groups explicit so their common lifecycle remains easy to follow.
  const rulerNodes = [...document.querySelectorAll(".cl-ruler[data-src]")];
  const richardsonNodes = [...document.querySelectorAll(".cl-richardson[data-src]")];
  if (!rulerNodes.length && !richardsonNodes.length) return;
  ...
  // Loading, visibility, theme lookup, and canvas primitives are kept here;
  // each builder below owns its DOM scaffold, geometry, drawing, and events.
  const cache = {};
  function load(src) {
  ...
  // ---- Page lifecycle --------------------------------------------------
  // Fetch near the viewport, reuse one request per data URL, and keep failure
  // handling outside builders. Each builder marks its mount ready once its
  // final synchronous scaffold exists.
  ...
  mountWhenVisible(rulerNodes, buildRuler, "Couldn’t load the coastline data.");
  mountWhenVisible(richardsonNodes, buildRichardson,
    "Couldn’t load the Richardson data.");
})();
