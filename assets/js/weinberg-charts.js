...
(function () {
  ...
  function buildCollocations(node, d) {
    ...
    function setCaption() {
      ...
      const base = editorialCopy(node, "Caption");
      if (filter === "all") {
      ...
      count.textContent = n;
      const category = el("b");
      category.textContent = cats[filter].label.toLowerCase();
      cap.replaceChildren(count, " of the top 20 are ", category, ". ", base);
    }
  ...
  function mountCharts(nodes) {
    nodes.forEach((node) => {
      ...
      load(src).then((data) => {
        // Replace viz-mount's no-JavaScript caption before the hydrated builder
        // adds its authored caption, keeping the loaded figure to one caption.
